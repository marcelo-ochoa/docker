...
if [ ! -f ${PERSISTENT_DATA}/DATABASE_IS_SETUP ]; then
	...
	exit
EOF
	if [ ! -f ${PERSISTENT_DATA}/OLS_IS_INSTALLED ]; then
		su - oracle -c "echo Install actions begin - $(date)" >> ${PERSISTENT_DATA}/install-ols.log
		su - oracle -c "loadjava -v -r -s -g PUBLIC -u sys/oracle@$HOSTNAME /home/oracle/ODCI.jar" >> ${PERSISTENT_DATA}/install-ols.log
		su - oracle -c "cd /home/oracle/ols;ant -Ddb.str=$HOSTNAME -Ddb.tbl=SYSAUX -Ddba.usr=sys -Ddba.pwd=oracle install-ols" >> ${PERSISTENT_DATA}/install-ols.log
		su - oracle -c "echo Install actions end - $(date)" >> ${PERSISTENT_DATA}/install-ols.log
		if [ $? -eq 0 ]; then
			touch ${PERSISTENT_DATA}/OLS_IS_INSTALLED
		fi
	fi
fi
